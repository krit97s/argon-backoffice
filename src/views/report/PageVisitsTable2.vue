<template>
  ...
  <b-card body-class="p-0" header-class="border-0">
    <template v-slot:header>
      <b-row align-v="center">
        <b-col class="d-flex align-items-center">
          <h3 class="mb-0">รายงานผลลัพธ์การเติม</h3>
    ...
    <el-table class="table-responsive table text-nowrap" :data="tableData" header-row-class-name="thead-light">
      ...
      <el-table-column label="Response" prop="response" min-width="250px" align="center">
        <template v-slot="{ row }">
          {{ row.response }}
        </template>
      </el-table-column>
      <el-table-column label="status" min-width="130px" prop="status" align="center">
        <template v-slot="{ row }">
          <b-badge pill :variant="row.status == 2 ? 'success' :  'danger' "> {{
            row.status
              == 2 ? 'สำเร็จ'  : 'ล้มเหลว (คืนเงิน)'
          }}</b-badge>
        </template>
      </el-table-column>
