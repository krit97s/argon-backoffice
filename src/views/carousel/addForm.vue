    ...
    <validation-observer v-slot="{ handleSubmit }" ref="formValidator">
      <b-form @submit.prevent="handleSubmit(submitForm)">
        ...
        <div class="pl-lg-4">
          <b-row>
            <b-col lg="12">
              <base-input :rules="{ required: true }" name="Name" label="ลิงค์รูป" placeholder="กรุณาวางลิงค์รูป"
                v-model="form.src">
              </base-input>
            </b-col>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      const response = await this.$store.dispatch("carousel/addCarousel", this.form)
      if (response.status) {
        await this.$notify({
          title: 'สำเร็จ !',
