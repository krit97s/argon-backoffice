<template>
  ...
  <b-card body-class="p-0" header-class="border-0">
    ...
    <el-table class="table-responsive table text-nowrap" :data="tableData" header-row-class-name="thead-light">
      ...
      <el-table-column label="src" prop="type" min-width="150px">
        <template v-slot="{ row }">
          <a :href="row.src" class="text-primary" target="_blank">{{ row.src }}</a>
        </template>
      </el-table-column>
        ...
        <template v-slot="{ row }">
          <div class="text-center">
            <base-button type="primary" size="sm"
              @click="$router.push(`/editcarousel?id=${row.id}`)">แก้ไข</base-button>
...
export default {
  ...
  methods: {
    async removeOptionById(id) {
      ...
      if (confirm) {
        const response = await this.$store.dispatch("carousel/removeCarousel", id)
        if (response.status) {
          await this.$notify({
            title: 'สำเร็จ !',
          ...
          await this.$store.dispatch("carousel/onFetchCarousel", { page: 1, perPage: this.perPage })
        } else {
          this.$notify({
            title: 'ล้มเหลว !',
            text: response.response.msg,
