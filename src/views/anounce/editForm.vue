...
export default {
  computed: {
    ...mapState({
      anounce: (state) => state.anounce.anounce,
    })
  },
  created() {
    this.fetchData()
  ...
  methods: {
    async fetchData() {
      await this.$store.dispatch("anounce/onFetchAnounceById", { id: this.$route.query.id })
    },
    async submitForm() {
      const response = await this.$store.dispatch("anounce/updateAnounce", this.anounce)
      if (response.status) {
        await this.$notify({
          title: 'สำเร็จ !',
