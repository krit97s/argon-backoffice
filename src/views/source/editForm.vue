  ...
  <card>
    ...
    <validation-observer v-slot="{ handleSubmit }" ref="formValidator">
      <b-form @submit.prevent="handleSubmit(submitForm)">
        <h6 class="heading-small text-muted mb-4">ข้อมูลแหล่งที่มา</h6>
        <div class="pl-lg-4">
          <b-row>
            <b-col lg="12">
...
export default {
  ...
  methods: {
    async fetchData() {
      await this.$store.dispatch("source/onFetchSourceById", { id: this.$route.query.id })
    },
    async submitForm() {
      const response = await this.$store.dispatch("source/updateSource", this.source)
      if (response.status) {
        await this.$notify({
          title: 'สำเร็จ !',
