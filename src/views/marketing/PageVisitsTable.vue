<template>
  ...
  <b-card body-class="p-0" header-class="border-0">
    ...
    <el-table class="table-responsive table text-nowrap" :data="tableData" header-row-class-name="thead-light">
      ...
      <el-table-column label="Amount" prop="name" min-width="150px">
        <template v-slot="{ row }">
          {{ row.name }}
        </template>
      </el-table-column>
      <el-table-column label="url" prop="name" min-width="150px">
        <template v-slot="{ row }">
          {{ row.url }}
        </template>
      </el-table-column>
      <el-table-column label="action" prop="status" align="center" min-width="150px">
        <template v-slot="{ row }">
          <div class="text-center">
            <base-button type="primary" size="sm" @click="$router.push(`/editsource?id=${row.id}`)">แก้ไข</base-button>
...
export default {
  ...
  methods: {
    ...
    async removeOptionById(id) {
      ...
      if (confirm) {
        const response = await this.$store.dispatch("source/removeSource", id)
        if (response.status) {
          await this.$notify({
            title: 'สำเร็จ !',
          ...
          await this.$store.dispatch("source/onFetchSource", { page: 1, perPage: this.perPage })
        } else {
          this.$notify({
            title: 'ล้มเหลว !',
            text: response.response.msg,
