  ...
  <card>
    <b-row align-v="center" slot="header">
      <b-col cols="12" class="d-flex align-items-center">
        <h3 class="mb-0">แก้ไขรายการลงทุน</h3>
    ...
    <validation-observer v-slot="{ handleSubmit }" ref="formValidator">
      <b-form @submit.prevent="handleSubmit(submitForm)">
        <h6 class="heading-small text-muted mb-4">ข้อมูลรายการลงทุน</h6>
        <div class="pl-lg-4">
          <b-row>
            <b-col lg="12">
...
export default {
  ...
  methods: {
    async fetchData() {
      await this.$store.dispatch("category/onFetchCategoryById", { id: this.$route.query.id })
    },
    async submitForm() {
      const response = await this.$store.dispatch("category/updateCategory", this.category)
      if (response.status) {
        await this.$notify({
          title: 'สำเร็จ !',
